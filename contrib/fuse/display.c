/* display.c: Routines for printing the Spectrum screen
   ...
   $Id: display.c,v 1.14 2001/10/03 22:37:00 pak21 Exp $
...
void display_dirty(WORD address, BYTE data)
{
	...
	{		/* 0x5800 = first attributes byte */
		if( read_screen_memory( address - 0x4000 ) != data )
...
int display_refresh_all_dirty( void )
{
	int screen_y, x;
	BYTE data, ink, paper;
	...
	for( screen_y = 0; screen_y < 192; screen_y++ )
	{
		if( display_is_dirty[screen_y] ) 
		{
		  ...
		  for( x=0;
		   ...
		   {
				/* Skip to next 8 pixel chunk if this chunk is clean */
				if( ! ( display_is_dirty[screen_y] & 0x01 ) ) 
					continue;
